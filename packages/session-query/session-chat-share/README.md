...
The dialog lists the Session's shareable messages (append-origin `user/message` and `assistant/message` text) newest-last, lets the user pick an inclusive range with the From/To selects or by clicking message rows, chooses Markdown or HTML, previews the rendered artifact, and then copies it to the clipboard or downloads it. The rendered artifacts are plain text — Markdown keeps the message text verbatim under role headers; HTML is a self-contained page with basic rendering (paragraphs and fenced code blocks) that any browser can open. Images are represented by an `[image]` marker; tool calls, tool results, boundary markers, and compaction-replaced copies are excluded.
