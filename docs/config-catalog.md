...
- `@deepseek-ai/dsh-session-chat-share` — requires `commands` ([`packages/session-query/session-chat-share/src/index.ts`](../packages/session-query/session-chat-share/src/index.ts))
