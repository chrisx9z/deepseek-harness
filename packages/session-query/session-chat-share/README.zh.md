...
对话框按最新在前列出会话中可分享的消息（追加来源的 `user/message` 与 `assistant/message` 文本），用户可通过 From/To 下拉框或点击消息行选择闭区间范围，选择 Markdown、HTML 或 TXT，预览渲染结果，然后复制到剪贴板或下载。渲染产物是纯文本——Markdown 在角色标题下原样保留消息文本；HTML 是自包含页面，提供基础渲染（段落和围栏代码块），任何浏览器都能打开；TXT 是去掉所有标记的同一内容。图片以 `[image]` 标记表示；工具调用、工具结果、边界标记和压缩替换副本均被排除。
