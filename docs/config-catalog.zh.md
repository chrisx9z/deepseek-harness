...
- `@deepseek-ai/dsh-session-chat-share` — 需要 `commands`（[`packages/session-query/session-chat-share/src/index.ts`](../packages/session-query/session-chat-share/src/index.ts)）
