...
行菜单自带三个动词——Rename、Fork 与 Archive。功能包可通过本包提供的 `sessionRowMenu` 服务追加更多行：`register(action)` 返回注销函数，贡献按 `order`（再按 id）排序并追加在三个核心动词之后，以函数给出的标签在每次渲染时读取（因而跟随注册方的 locale），菜单关闭后执行 `action.run(sessionId)`。服务快照在两次注册之间保持身份稳定，因此浏览区只在贡献集合变化时重渲染。会话分享是参考贡献者：它通过 `ctx.inject` 注册 **分享** 与 **保存 TXT**，因此没有该注册表的组合仍然以 Header 操作为入口。
